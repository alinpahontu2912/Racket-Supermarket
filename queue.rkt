...
(define (queue-empty? q)
  (zero? (+ (queue-size-l q) (queue-size-r q) )))
...
(define (enqueue x q)
  ...
  (match q [(queue left right size-l size-r)  (make-queue (rotate left (append (list x) right) empty-stream) '() (+ (+ size-l 1) size-r) 0)])
  (match q [(queue left right size-l size-r)  (make-queue left (append (list x) right) size-l (+ size-r 1))])))
...
(define (dequeue q)
  (if (> (queue-size-r q) (- (queue-size-l q) 1))
  (match q [(queue left right size-l size-r)  (make-queue (stream-rest (rotate left right empty-stream)) '() (- (+ size-l size-r) 1) 0)])
  (match q [(queue left right size-l size-r)  (make-queue (stream-rest left) right (- size-l 1) size-r)])
 ))
...
(define (top q)
  ...
  (list-ref (queue-right q) (- (length (queue-right q)) 1))
